    ...
    for blk in blocks:
        ...
        for ln in lines:
            ...
            if splits[0] == 'flags': # status
                ...
                keys = splits[::2]
                vals = splits[1::2]    
                iface['ipv4'] = dict(list(zip(keys, vals))) # inet, netmast, [broadcast]
            elif splits[0] == 'inet6': #ipv6
                keys = splits[::2]
                vals = splits[1::2]
                iface['ipv6'] = dict(list(zip(keys, vals))) # inet6, prefixlen, scopeid
            ...
            # statistics
                ...
                keys = [splits[0] + ' ' + k for k in splits[1::2]]        
                vals = splits[2::2]
                iface['statistics'].append(dict(list(zip(keys, vals))))
        #print iface
        interfaces.append(iface)
    # write the docker-images brief info
    with open('%s/interfaces_out.json' % subpath, 'w') as fo:
        print(json.dumps(interfaces, sort_keys=True, indent=4), file=fo) # without indent can save space
