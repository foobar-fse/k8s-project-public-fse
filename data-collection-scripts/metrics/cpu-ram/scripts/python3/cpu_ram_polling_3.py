...
def cpu_mem_poll(outpath):
    ...
    res['task'] = dict(list(zip(task_key, task_val)))
    ...
    res['cpu'] = dict(list(zip(cpu_key, cpu_val)))
    ...
    res['ram'] = dict(list(zip(ram_key, ram_val))) 
    ...
    res['swap'] = dict(list(zip(swap_key, swap_val)))
    ...
    for line in lines[1:]:
        ...
        proc = dict(list(zip(proc_key, proc_val)))
        procs.append(proc)
    res['process'] = procs

    # write the cpu+ram metric info
    with open('%s/cpu_ram_metric_out.json' % subpath, 'w') as fo:
        print(json.dumps(res, sort_keys=True, indent=4), file=fo) # without indent can save space
