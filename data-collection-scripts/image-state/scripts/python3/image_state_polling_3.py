...
def image_state_poll(outpath):
    ...
    with open('%s/docker_images_out.json' % subpath, 'w') as fo:
        print(json.dumps(images, sort_keys=True, indent=4), file=fo) # without indent will save space

    with open('%s/all_images.json' % subpath, 'w') as fo1:
        print(json.dumps(image_details, sort_keys=True, indent=4), file=fo1)
